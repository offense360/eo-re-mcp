...
| [#46](https://github.com/offense360/eo-re-mcp/issues/46) | IDA | A failed `parse_type_declaration` still registers the named type in the local type library (`parse_decls` mutates before reporting errors); the next correct declaration of that name reports a merge, not a creation (found while verifying #32) | Open |
 (enhancement; found in the 2026-09-06 real-usage run on `curl.exe` / WSL `curl`) | Open |

...
| [#49](https://github.com/offense360/eo-re-mcp/issues/49) | Ghidra | `set_type` silently replaces instructions with data at a function entry or inside a function body (`status: ok`, only `undo` recovers); should refuse or warn unless forced (found while verifying #36) | Open (low priority) |
| [#50](https://github.com/offense360/eo-re-mcp/issues/50) | IDA | `capabilities.assembler` is always false on x86: `ida_idp.get_idp_name()` returns `pc` on IDA 9.4 but the session compares with `metapc` (inherited; informational only, found while verifying #38) | Open (low priority) |
...
| [#46](https://github.com/offense360/eo-re-mcp/issues/46) | IDA | A failed `parse_type_declaration` still registers the named type when only the trailing `;` is missing (IDA's `parse_decls` stores a complete body before counting the error; unknown types / unbalanced braces store nothing); the next correct declaration of that name reports a merge, not a creation. Inherited: reproduced on upstream 799239f with the same input. Low severity: nothing is corrupted and each failed call has its own undo point since #26. Fix: drop the new ordinals (or roll back to the call's undo point) before raising. | Open (low priority) |
 (enhancement; found in the 2026-09-06 real-usage run on `curl.exe` / WSL `curl`) | Open |

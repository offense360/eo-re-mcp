...
| [#24](https://github.com/offense360/eo-re-mcp/issues/24) | core | Design question: `open_database(run_auto_analysis=True)` on an already-analyzed database forces one full re-analysis (IDA since #23, Ghidra always); decide whether to skip when the worker reports `analyzed`. Not a double pass (first wording corrected). | Open, low priority |

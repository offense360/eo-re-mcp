...
| [#24](https://github.com/offense360/eo-re-mcp/issues/24) | core | `open_database(run_auto_analysis=True)` runs auto-analysis twice now that an explicit `analyze_database` re-plans (seed ignored on explicit requests, #8 rule) | Open |

...
| [#47](https://github.com/offense360/eo-re-mcp/issues/47) | Ghidra | `parse_type_declaration` passed the CParser message through with CR LF breaks, duplicate lines and the Java exception class prefix; `clean_parser_message` now yields one line (`Encountered "<EOF>" at line 1, column 23. Was expecting one of: "}" ...`). | Fixed (72cc186) |
| [#48](https://github.com/offense360/eo-re-mcp/issues/48) | all | Distributions renamed to `eo-re-mcp`, `eo-re-mcp-core`, `eo-re-mcp-ida`, `eo-re-mcp-ghidra` with the fork's own versioning from 1.0.0 (the `ida-mcp` alias package dropped); import names, CLI names, entry points and state directories unchanged. | Fixed (fed5c6e) |
| [#49](https://github.com/offense360/eo-re-mcp/issues/49) | Ghidra | `set_type` silently replaced instructions with data at a function entry or inside a function body; it now raises `InvalidArgument` naming the function (use `set_function_type`, `delete_function`, or `force=True`). | Fixed (2da4ff3) |
| [#50](https://github.com/offense360/eo-re-mcp/issues/50) | IDA | `capabilities.assembler` was always false on x86 because IDA 9.4 reports the processor as `pc` while the session compared with `metapc` (inherited); both names are accepted now. | Fixed (fbed693) |
...
| [#35](https://github.com/offense360/eo-re-mcp/issues/35) | IDA | `list_local_types` had no `filter_pattern` while Ghidra's did; both now accept a regex on the type name. | Fixed (8e72998) |
| [#36](https://github.com/offense360/eo-re-mcp/issues/36) | Ghidra | `set_type` at a function entry with a C prototype failed with `Unknown data type: '<whole prototype>'`; it now raises `InvalidArgument` naming the function and pointing at `set_function_type`. | Fixed (72cc186) |
| [#37](https://github.com/offense360/eo-re-mcp/issues/37) | IDA | `find_code_by_string` returned a non-paginated `results` list without the referencing instruction; it now returns the same `items`/`total`/`has_more` envelope as Ghidra with `code_address` per item (whole database scanned, `total_strings_scanned`/`unique_functions` kept as extras). | Fixed (8e72998) |
| [#38](https://github.com/offense360/eo-re-mcp/issues/38) | all | `get_database_info` field sets differed (Ghidra lacked `entry_point`; IDA lacked `image_base`, `endian`, `compiler_spec`, `capabilities`); both backends now share that core set, backend extras stay. | Fixed (72cc186 Ghidra, 8e72998 IDA) |
| [#39](https://github.com/offense360/eo-re-mcp/issues/39) | Ghidra | `undo`/`redo` responses said only `{"action": "undo"}`; they now carry `label` (the reverted call's transaction label, same field as IDA since #26), `remaining_undo` and `remaining_redo`. | Fixed (b840608) |
| [#40](https://github.com/offense360/eo-re-mcp/issues/40) | IDA | `get_pseudocode_line_map` called `find_item_coords` once per ctree item (~1 ms each; 6.6–9 s on a 1,412-line function); the map is now built in one pass over the pseudocode `COLOR_ADDR` anchors (0.05 s) and also covers label and continuation lines. | Fixed (aa395ae) |
| [#41](https://github.com/offense360/eo-re-mcp/issues/41) | all | `disassemble_function` / `decompile_function` returned the whole function with no cap (478 KB / 123 KB for the largest ELF function; upstream never had one). Both backends now page: `offset`/`limit` over instructions (default 500) and 0-based `start_line`/`max_lines` over pseudocode lines (default 2000, aligned with IDA's `get_pseudocode_line_map`), with whole-function totals, `has_more`, `next_line` and a `note`; no absolute ceiling. Ghidra caches the decompiled text per function (validated by `getModificationNumber()`). | Fixed (fb561fd Ghidra, 8196685 IDA) |
| [#42](https://github.com/offense360/eo-re-mcp/issues/42) | Ghidra | `decompiled_code` / `export_all_pseudocode.pseudocode` used CR LF on Windows with a padding blank line at each end; now LF only with no padding, the shape IDA returns. | Fixed (06c0c1e) |
| [#43](https://github.com/offense360/eo-re-mcp/issues/43) | Ghidra | EXTERNAL-space addresses (imported symbols) were rendered as slot offsets (`free` at `0xB0`) by `get_call_graph` and `get_imports`, and dropped by `get_xrefs_from` after #30; one renderer now shows them as `EXTERNAL:<library>::<name>` with `library` and `thunk_address` (the PE stub / ELF `.plt.sec` entry in real memory, null for IAT-only imports), external callees appear once, and `get_imports.address` is the thunk. | Fixed (2920a00) |
...
| [#45](https://github.com/offense360/eo-re-mcp/issues/45) | Ghidra | `entry_point_count` differed between `get_database_info` (7) and `analyze_database` (5) on the curl ELF (three predicates); all tools and resources now use `SymbolTable.getExternalEntryPointIterator()` (one entry per address, primary symbol name) via a shared `entry_points()` helper. | Fixed (2920a00) |
| [#46](https://github.com/offense360/eo-re-mcp/issues/46) | IDA | A failed `parse_type_declaration` left the named type registered when only the trailing `;` was missing (inherited; IDA stores a complete body before counting the error); the tool now deletes the ordinals added by the failed call and names them in the error (`discarded partially registered type(s): E46`). | Fixed (fbed693) |

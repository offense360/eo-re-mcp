...
- `IDAServer.tool()` (in `re_mcp_ida.server`) wraps every tool registered with `readOnlyHint=False` so that `ida_undo.create_undo_point("re-mcp", <tool name>)` runs on the main thread right before the tool body; `undo` therefore reverts exactly one tool call and reports that name as `label` (#26). The exempt names (`undo`, `redo`, `analyze_database`, `reanalyze_range`, `open_database`, `save_database`, `close_database`) live in `UNDO_POINT_EXEMPT` next to the class.

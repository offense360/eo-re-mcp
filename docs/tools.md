...
| `set_type` | Apply a C type declaration at an address (a function prototype at a function entry is rejected with a pointer to `set_function_type`; refuses to overwrite instructions inside a function unless `force=True`). |
